...
| **Membership-page video** | `JOIN_VIDEO_ID`, the ID after `v=` in a YouTube link (a whole link works too). Currently `zrjh6igPAfw`. Set it blank to show no video. On Render: Environment → add `JOIN_VIDEO_ID` → Save (it redeploys). |
...
- **Adding music:** open `/admin` > **Music store > Add music** and drag songs or whole album folders in (a loose file is a song, a folder is an album). Progress bars, two at a time, and dragging the same folder again skips what is already there, so a big batch can be resumed. The same panel lists everything in the store (searchable, with Remove) and shows disk space used and free. The biggest single file is `MAX_STORE_FILE_MB` (default 100). The store page has a search box once there are more than 24 items.
